...
function extractProcess(responseToClient) {
    ...
    data = request(defaultUrl, function (err, res, html) {
        if (!err) {
            const $ = cheerio.load(html);
            data = $('#hotellist_inner').children();
            dataString = data.toString();
            responseToClient.json( {'dataLength': dataString.length} );
            dataFromExtractProcess = data;
...
function etlProcess(responseToClient) {
    request(defaultUrl, function (err, res, html) {
        if (!err) {
            const $ = cheerio.load(html);
